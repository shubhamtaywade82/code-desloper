...
### 3.8 Avoid `default_scope`
**Smell:** Using `default_scope` for ordering or filtering.
**Fix:** Use explicit named scopes. `default_scope` is notoriously difficult to override and often leaks into unintended queries.

### 3.9 Use `Time.zone.now`
...
### 3.10 Batched Finders for Large Collections
